...
| Change who can see or do what | `backend-context.md` → Roles, then `applyVisibilityFilter` |
...
backend's `planning` and `reporting` modules. There are no tests and no CI.
